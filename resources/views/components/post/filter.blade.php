    ...
    <div class="bg-white rounded-top">
        <div class="d-lg-flex mx-4 border-bottom">
            ...
            <div class="d-flex justify-content-between align-items-start py-3">
                @mobile
                    <button type="button" class="btn btn-success px-4" data-bs-toggle="modal" data-bs-target="#groupsModal" style="background-color: var(--bs-orange);border: var(--bs-btn-border-width) solid var(--bs-orange);">{{ fs_db_config('group_name') }}</button>
                @endmobile

                @if (fs_user()->check())
                    @if (fs_db_config('forumq_quick_publish'))
                    ...
                    @endif
                @else
                    <button class="btn btn-primary px-4" type="button" data-bs-toggle="modal" data-bs-target="#postTipModal">{{ fs_db_config('publish_post_name') }}</button>
                @endif
            </div>
        </div>
    </div>

                        ...
                        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">
                            @foreach($tree['groups'] as $treeGroup)
                                <li class="d-flex justify-content-between align-items-start">
                                    <a href="{{ fs_route(route('fresns.group.detail', ['gid' => $treeGroup['gid']])) }}" class="link-dark d-inline-flex text-decoration-none rounded py-2 @if ($treeGroup['gid'] == $groupGid) active @endif">{{ $treeGroup['name'] }}</a>
